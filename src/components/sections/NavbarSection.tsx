...
const NavbarSection: React.FC = () => {
  ...
  return (
    <nav className={`bg-gray-800 text-white shadow-lg ${styles.navbar} ${isSticky ? styles.navbarSticky : ""}`}>
      <div className="container mx-auto px-6 py-3 md:flex md:justify-between md:items-center">
        ...
        <div className={`${isMobileMenuOpen ? "block" : "hidden"} md:flex items-center`}>
          <div className="flex flex-col md:flex-row md:mx-6">
            <Link href="/" passHref>
              <span className={`my-1 text-sm md:mx-4 md:my-0 ${isActive("/") ? "text-green-500" : "text-gray-200"} hover:text-green-500 cursor-pointer`}>
                {t("home")}
              </span>
            </Link>
            <Link href="/services" passHref>
              <span className={`my-1 text-sm md:mx-4 md:my-0 ${isActive("/services") ? "text-green-500" : "text-gray-200"} hover:text-green-500 cursor-pointer`}>
                {t("services")}
              </span>
            </Link>
            <Link href="/about" passHref>
              <span className={`my-1 text-sm md:mx-4 md:my-0 ${isActive("/about") ? "text-green-500" : "text-gray-200"} hover:text-green-500 cursor-pointer`}>
                {t("about")}
              </span>
            </Link>
            <Link href="/contact" passHref>
              <span className={`my-1 text-sm md:mx-4 md:my-0 ${isActive("/contact") ? "text-green-500" : "text-gray-200"} hover:text-green-500 cursor-pointer`}>
                {t("contact")}
              </span>
            </Link>
            {isUserAdmin && (
          ...
          <div className="flex items-center md:ml-6">
            {isLoggedIn ? (
              ...
              <>
                <Link href="/login" passHref>
                  <AnimatedButton
                    className={`px-3 py-2 rounded-md text-sm font-medium mr-4 ${isActive("/login") ? "text-gray-200 bg-green-600 hover:bg-gray-700" : "text-gray-200 hover:bg-green-600 bg-gray-700"}`}
                  >
                    {t("login")}
                  </AnimatedButton>
                </Link>
                <Link href="/register" passHref>
                  <AnimatedButton
                    className={`px-3 py-2 rounded-md text-sm font-medium ${isActive("/register") ? "text-gray-200 bg-green-600 hover:bg-gray-700" : "text-gray-200 hover:bg-green-600 bg-gray-700"}`}
                  >
                    {t("register")}
                  </AnimatedButton>
                </Link>
            ...
            <button
              ...
              className={`focus:outline-none mx-2 ${styles.buttonTransition} ${isButtonPressed ? styles.buttonPressed : ""}`}
            >
              {locale === "en" ? (
                <Image
